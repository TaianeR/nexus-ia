...
# ─────────────────────────────────────────────
# Busca em tempo real (Groq Compound)
# ─────────────────────────────────────────────
PALAVRAS_CHAVE_BUSCA = [
    "hoje", "agora", "atual", "atualmente", "recente", "recentes",
    "notícia", "notícias", "resultado", "resultados", "jogo", "jogos",
    "placar", "cotação", "dólar", "preço", "preços", "quem ganhou",
    "quem é o atual", "última", "últimas", "clima", "tempo em",
    "previsão do tempo", "quando foi", "quando é", "que dia é hoje",
    "quanto está", "quanto custa", "eleição", "eleições", "campeonato",
]


def precisa_buscar(pergunta: str) -> bool:
    p = pergunta.lower()
    return any(chave in p for chave in PALAVRAS_CHAVE_BUSCA)


def buscar_tempo_real(pergunta: str) -> str:
    """Usa o groq/compound (com busca na web embutida) para trazer informação atual."""
...
if not st.session_state.messages:
    ...
    <div style='text-align:center; margin-bottom: 1.5rem;'>
        ...
        <span class='suggestion-chip'>📰 Pergunte algo atual (notícias, jogos, cotação)</span>
    </div>
    """, unsafe_allow_html=True)
...
for msg in st.session_state.messages:
    if msg["role"] == "user":
        ...
        tag_html = ""
        if msg.get("busca_ok"):
            tag_html = "<div class='busca-tag'>🔎 busca em tempo real usada</div><br>"
        elif msg.get("busca_erro"):
            tag_html = f"<div class='busca-erro-tag'>⚠️ busca falhou: {msg['busca_erro']}</div><br>"
        st.markdown(f"""
        <div class='message-row nexus'>
            <div class='avatar avatar-nexus'>✦</div>
            <div class='bubble bubble-nexus'>{tag_html}{msg['content']}</div>
        </div>""", unsafe_allow_html=True)
...
if pergunta:
    ...
    # Tenta buscar informação atual se a pergunta parecer precisar disso
    contexto_busca = ""
    busca_ok = False
    busca_erro = None
    if precisa_buscar(pergunta):
        with st.spinner("🔎 Buscando informação atualizada..."):
            try:
                contexto_busca = buscar_tempo_real(pergunta)
                busca_ok = True
    ...
    if contexto_busca:
        system_prompt_final += (
            "\n\nVocê tem acesso à seguinte informação atualizada, obtida agora via busca na web. "
            "Use-a para responder com dados reais e atuais, sem inventar nada:\n"
            f"{contexto_busca}"
        )
    elif busca_erro:
        system_prompt_final += (
            "\n\nA busca em tempo real falhou agora. Avise educadamente ao usuário que não "
            "foi possível verificar a informação mais recente neste momento e responda com o "
            "que souber, deixando claro que pode estar desatualizado."
        )
    ...
    if busca_ok:
        ...
        tag_html = f"<div class='busca-erro-tag'>⚠️ busca falhou: {busca_erro}</div><br>"

    st.markdown(f"""
    <div class='message-row nexus'>
        <div class='avatar avatar-nexus'>✦</div>
        <div class='bubble bubble-nexus'>{tag_html}{conteudo}</div>
    </div>""", unsafe_allow_html=True)
