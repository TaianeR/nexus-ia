...
<style>
    ...
    .message-row { display: flex; align-items: flex-start; gap: 14px; margin-bottom: 1.75rem; padding-bottom: 1.75rem;
        border-bottom: 1px solid rgba(255,255,255,0.05); animation: slideIn 0.25s ease-out; width: 100%; }
    @keyframes slideIn { from { opacity: 0; transform: translateY(10px);} to { opacity: 1; transform: translateY(0);} }
    .message-row.user, .message-row.nexus { flex-direction: row; }
    .avatar { width: 32px; height: 32px; border-radius: 50%; display: inline-flex; align-items: center; justify-content: center;
        font-size: 1rem; flex-shrink: 0; overflow: hidden; margin-top: 2px; }
    ...
    .bubble { padding: 4px 0; border-radius: 0; max-width: 100%; width: 100%; font-size: 0.95rem; line-height: 1.75; word-wrap: break-word; }
    .bubble-nexus { background: transparent; border: none; color: #e2e8f0; }
    .bubble-user { background: transparent; border: none; color: #f1f5f9; font-weight: 500; }
    .attachment-img { max-width: 280px; max-height: 280px; border-radius: 12px; margin-bottom: 10px; display: block; object-fit: cover; }
    .attachment-doc { display: inline-flex; align-items: center; gap: 6px; background: rgba(255,255,255,0.04); border: 1px solid rgba(255,255,255,0.1);
        border-radius: 10px; padding: 6px 12px; margin-bottom: 10px; font-size: 0.8rem; }
    .generated-img { max-width: 100%; width: 460px; border-radius: 14px; margin-bottom: 6px; display: block; border: 1px solid #1e1e3f; }
...
def extrair_url_imagem(html_img):
    """Extrai a URL de dentro de uma tag <img src='...'>"""
    m = re.search(r'src="([^"]+)"', html_img or "")
    return m.group(1) if m else None

@st.cache_data(show_spinner=False, ttl=3600)
def baixar_imagem_bytes(url):
    """Baixa os bytes de uma imagem gerada, para permitir o download no app"""
    try:
    ...
    with col2:
        for idx_msg, msg in enumerate(st.session_state.messages):
            ...
            if msg["role"] == "user":
                ...
                if msg.get("is_image") and msg.get("content"):
                    url_img = extrair_url_imagem(msg["content"])
                    if url_img:
                        img_bytes = baixar_imagem_bytes(url_img)
                        if img_bytes:
                            st.download_button("📥 Baixar imagem", data=img_bytes,
                                file_name=f"nexus_imagem_{idx_msg}.png", mime="image/png", key=f"img_hist_{idx_msg}")
                elif not msg.get("is_image") and msg.get("content"):
        ...
        if entrada:
            ...
            with st.spinner(""):
                ...
                try:
                    ...
                    if comando_tom:
                        ...
                        indicadores_busca = ["hoje", "ontem", "noticia", "notícia", "quem é", "quem foi", "resultado",
                                              "jogo", "ganhou", "perdeu", "placar", "copa", "campeonato",
                                              "atual", "tempo", "clima", "preço", "dolar", "euro", "cotação",
                                              "últimas", "ultimas", "pesquise", "busca", "quando", "onde fica", "quanto custa"]
                        contexto_web = ""
            ...
            if is_image_flag:
                url_img_novo = extrair_url_imagem(conteudo)
                if url_img_novo:
                    img_bytes_novo = baixar_imagem_bytes(url_img_novo)
                    if img_bytes_novo:
                        st.download_button("📥 Baixar imagem", data=img_bytes_novo,
                            file_name="nexus_imagem_nova.png", mime="image/png", key="img_novo")
            else:
